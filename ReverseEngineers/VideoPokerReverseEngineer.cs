...
internal class VideoPokerReverseEngineer
{
    ...
    internal VideoPokerReverseEngineer(ProvablyFairAlgorithm algorithm)
    {
    ...
    private List<double> GetResultNumbers()
    {
        ...
        for (double i = BytesToNumberMultiplier; i >= 1; i--)
        {
            multipliers.Add(i);
        }
    ...
    private List<string> NumbersToDeck(List<double> numbers)
    {
        ...
        Dictionary<int, string> playingCardsCopy = new Dictionary<int, string>(PlayingCards);
        foreach (var number in numbers)
        {
            int truncatedNumber = (int)number;
            int key = playingCardsCopy.Keys.ElementAt(truncatedNumber);
            finalDeck.Add(playingCardsCopy[key]);
            playingCardsCopy.Remove(key);
        }
        return finalDeck.GetRange(0,10);
    }
    
    internal List<string> GetVideoPokerDeck()
    {
